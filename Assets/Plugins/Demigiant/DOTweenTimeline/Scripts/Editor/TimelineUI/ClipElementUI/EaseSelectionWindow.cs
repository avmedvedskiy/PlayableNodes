...
namespace DG.Tweening.TimelineEditor.ClipElementUI
{
    ...
    {
        DOTweenClipTimeline _editor { get { return DOTweenClipTimeline.editor; } }
        ...
        List<DOTweenClipElement> _clipElementRefs;
        int _totEases;
            ...
            for (int i = 0; i < _totEases; ++i) {
                ...
                if (GUI.Button(btR.Expand(_Padding), GUIContent.none, DOEGUI.Styles.easePopup.btEase)) {
                    using (new DOScope.UndoableSerialization(false, true)) {
                        Ease mainEase = _clipElementRefs[0].ease;
                        ...
                        if (EaseMainTypeIsDifferent(mainEase, newEase)) {
                            ...
                            foreach (DOTweenClipElement clipElement in _clipElementRefs) {
                                clipElement.overshootOrAmplitude = overshootOrAmplitude;
                                clipElement.period = period;
                            }
                        }
                        foreach (DOTweenClipElement clipElement in _clipElementRefs) {
                            clipElement.ease = newEase;
                        }
                    }
                    this.editorWindow.Close();
                    DOTweenClipTimeline.Dispatch_OnClipChanged(DOTweenClipTimeline.clip);
                }
                Texture2D tex = _easeImgs[i];
                if (tex == null) Init(true); // I don't fully understand how this could be NULL but it sometimes happen after a recompilation
        ...
        public void Prepare(List<DOTweenClipElement> clipElements)
        {
            _clipElementRefs = clipElements;
            _selectedEases = new string[clipElements.Count];
            for (int i = 0; i < _selectedEases.Length; ++i) {
                _selectedEases[i] = clipElements[i].ease.ToString();
            }
        }
