...
namespace DG.Tweening.TimelineEditor
{
    internal class TimelineScrubber : ABSTimelineElement
    {
            ...
            case EventType.MouseDown:
                if (_scrubArea.Contains(Event.current.mousePosition)) {
                    ...
                    if (!_wasPreviewActive) {
                        Tween t = DOTimelinePreviewManager.StartPreview(editor, clip, false);
                        if (t == null) break;
                    }
                    _scrubbing = true;
                    UpdateScrubber();
        ...
        void DrawPlayMode()
        {
            if (clip.tween == null || !clip.tween.IsActive()) return;

            float duration = Application.isPlaying && clip.timeMode == TimeMode.DurationOverload
                ? TimelineUtils.GetClipDuration(clip)
                : clip.tween.Duration(false);
            float clipTime = clip.tween.ElapsedDirectionalPercentage() * duration;
            float tweenX = timelineShift.x + clipTime * settings.secondToPixels;
            // Delay info
            float delay = clip.tween.Delay();
            float elapsedDelay = clip.tween.ElapsedDelay();
