...
namespace DG.Tweening.Timeline.Core.Plugins
{
    public static class DOVisualPluginsManager
    {
        ...
        // LEGACY: Order of internal PluginData index is important and can't be changed (stored in clipElement to determine right plugin data)
        // UPDATE (v0.9.400): PluginData is now stored via unique GUID so order is not important anymore
        ...
        public static DOVisualTweenPlugin Editor_GetTweenPluginByPlugDataGuid(string plugDataGuid)
        {
            if (string.IsNullOrEmpty(plugDataGuid)) return null;
            foreach (DOVisualTweenPlugin plug in _TypeToTweenPlugin.Values) {
                for (int i = 0; i < plug.totPluginDatas; ++i) {
                    if (plug.pluginDatas[i].guid == plugDataGuid) return plug;
                }
            }
            return null;
        }
